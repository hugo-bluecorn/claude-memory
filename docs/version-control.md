...
main (protected, always deployable)
  ├── feature/phase-0-1-project-structure
  ├── feature/add-crop-status-update
...
|--------|---------|---------|
| `feature/` | New functionality, phase work | `feature/phase-1-domain-models` |
| `fix/` | Bug fixes | `fix/crop-sync-null-error` |
| `docs/` | Documentation only | `docs/add-contributing-guide` |
| `refactor/` | Code restructuring, no behavior change | `refactor/extract-sync-service` |
| `test/` | Test additions/fixes only | `test/add-crop-repository-tests` |
...
- Include phase number for phase work: `feature/phase-0-1-project-structure`
- Include ticket/issue number if applicable: `fix/issue-42-sync-timeout`

**Examples:**
```
✓ feature/phase-2-drift-tables
...
This project uses **milestone-based releases** aligned with the development roadmap phases.

### When to Release

Create a release after completing significant milestones:
- End of foundation phases (after Phase 0.9)
- Major feature completion (after Phase 6, 7, etc.)
- MVP completion (after Phase 12.5)
...
0.1.0  → First internal release (Phase 0 complete)
0.2.0  → Domain layer complete (Phase 1)
0.x.0  → Each phase milestone
1.0.0  → MVP release (Phase 12.5 complete)
1.0.1  → Patch/bugfix after MVP
1.1.0  → Post-MVP enhancement
```
...
5. **Create annotated tag:**
   ```bash
   git tag -a v0.2.0 -m "Release 0.2.0: Domain Layer Complete"
   ```
